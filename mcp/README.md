...
| `title` | 文字列 | イベント名（必須） |
| `tags` | 文字列配列 | タグ ID。ジャンル → シリーズ → 主催 の順。ジャンルの色がカレンダーの色になる（下記「タグの軸」） |
...
`tags` マスタの各タグは `group` を持ちます。

| group | 例 | 付け方 |
| --- | --- | --- |
| `genre` | ボカロ / アニソン / Vtuber / ゲーム / 歌い手 / その他 | 必ず 1 つ。色がカレンダーの色になる |
| `series` | `s-shadownight`, `s-utamata` など | シリーズものだけ。単発イベントには付けない |
| `org` | `external`（外部主催） | 外部主催のときだけ |

公開ページの絞り込みは「ジャンル」と「シリーズ」の 2 段で、段の中は OR、段どうしは AND です。
...
`admin.html` の画像アップロードも同じ方式です（`images/` に保存し、`events.json` にはパスだけ）。
`events.json` は訪問者全員が毎回ダウンロードするファイルなので、画像を base64 で埋め込むことはどちらの経路でもしません。
